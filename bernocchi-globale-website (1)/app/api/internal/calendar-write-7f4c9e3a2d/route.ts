...
function googleError(body: unknown) {
...
export async function GET() {
  ...
  try {
    ...
    if (!tokenRes.ok || !tokenBody?.access_token) return NextResponse.json({ ok: false, stage: 'auth', status: tokenRes.status, error: googleError(tokenBody) })
    ...
    if (!readRes.ok) return NextResponse.json({ ok: false, stage: 'read', status: readRes.status, error: googleError(readBody) })
    ...
    const insertRes = await fetch(`${base}/events`, {
      ...
      body: JSON.stringify({
        ...
        description: 'Temporary connectivity test; automatically removed.',
    ...
    if (!insertRes.ok || !insertBody?.id) return NextResponse.json({ ok: false, stage: 'insert', status: insertRes.status, error: googleError(insertBody) })
    ...
    return NextResponse.json({ ok: deleteRes.ok, stage: deleteRes.ok ? 'complete' : 'delete', auth: true, read: true, insert: true, delete: deleteRes.ok })
  } catch (error) {
